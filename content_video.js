  ...
  function attachToVideo(el) {
    try {
      ...
      function flush(reason) {
        try {
          // Build aggregated row only when appropriate.
          // Only use cursorSamples and keyboard samples collected since LAST flush.
          ...
          // durationSec is the TOTAL duration of the video, whereas active_interval_ms (new) would be the flush window.
          // For now, we'll keep video_duration_sec as the video property, but behavioral counts are incremental.
          const videoRow = {
            ts_start: playingStartTs || Date.now(),
            ts_end: now,
            video_session_id: sessionId || makeSessionId(),
            tab_id: null, // background will add sender.tab
          ...
          // INCREMENTAL: reset behavioral accumulators after every periodic or event-driven flush
          // This ensures each row shows only what happened since the last row.
          cursorSamples = [];
          ...
          // We do NOT reset cumulativePlayMs or sessionId here, they belong to the whole video session.
